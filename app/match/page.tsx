...
export default function MatchPage() {
    ...
    if (isLoading) return <div className="h-screen flex items-center justify-center font-black text-indigo-600 animate-pulse">MATCHING...</div>;

    return (
        <div className="flex flex-col h-screen bg-neutral-50 p-4 md:p-6 overflow-hidden">
            <header className="flex items-center gap-4 py-2 mb-4 flex-shrink-0">
            ...
            {!currentUser ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-6 max-w-lg mx-auto w-full">
                    {/* 카드 클릭 시 상대방 상세 프로필로 이동 */}
                    <div 
                        onClick={() => router.push(`/profile/${currentUser.userId}`)}
                        className="w-full bg-white rounded-[40px] shadow-2xl overflow-hidden border border-neutral-100 flex flex-col items-center p-10 text-center relative cursor-pointer hover:scale-[1.02] transition-all"
                    >
                        <div className="absolute top-6 right-6 bg-indigo-600 text-white px-4 py-1.5 rounded-full font-black text-lg">
                            {Math.round(currentUser.matchScore)}%
                        </div>
                        <div className="w-32 h-32 bg-indigo-50 rounded-full flex items-center justify-center text-6xl mb-6 shadow-inner">
                            {currentUser.avatar ? <img src={currentUser.avatar} className="w-full h-full rounded-full object-cover" alt="avatar" /> : "👤"}
                        </div>
                        <h2 className="text-3xl font-black text-gray-900 mb-1">{currentUser.nickname}</h2>
                        <p className="text-gray-400 font-bold mb-4">{currentUser.university || "세명대학교"} · {currentUser.mbti}</p>
                        <div className="flex flex-wrap justify-center gap-2">
                            {currentUser.interests.map((tag, i) => (
                                <span key={i} className="bg-indigo-50 text-indigo-600 px-4 py-1.5 rounded-full font-black text-sm">#{tag}</span>
                            ))}
                        </div>
                    </div>

                    <div className="flex items-center gap-10 pb-4">
                        <button onClick={() => handleAction('dislike')} className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-lg text-gray-400 hover:text-red-500 border border-neutral-100 transition-all active:scale-90">
                            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12"/></svg>
                        </button>
                        <button onClick={() => handleAction('like')} className="w-20 h-20 bg-indigo-600 rounded-full flex items-center justify-center shadow-xl text-white hover:bg-indigo-700 transition-all active:scale-90">
